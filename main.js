...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Cargar lista de países (ahora estática)
    loadCountries();
    ...
    registerForm.addEventListener('submit', function(e) {
        ...
        if (validateRegisterForm()) {
            ...
            const user = {
                ...
                isDeveloper: document.getElementById('password').value === DEV_PASSWORD,
        ...
        selectedFiles.forEach((file, index) => {
            ...
            reader.onload = function(e) {
                const fileData = {
                    ...
                    visibility: isPrivate ? 'private' : visibility, // Sobrescribir visibilidad si es privado
    ...
    function loadGalleryFiles() {
        ...
        getAllFiles()
            .then(files => {
                ...
                // Filtrar archivos privados (solo mostrar si son del usuario actual)
                files = files.filter(file => 
                    !file.isPrivate || file.userEmail === currentUser.email
                );
                ...
                files.forEach(file => {
                    ...
                    if (file.visibility === 'public' || file.userEmail === currentUser.email) {
                        ...
                        // Añadir indicador de privacidad si el archivo es privado
                        const privateBadge = file.isPrivate ? 
                            ...
                            <div class="card-body">
                                ...
                                ${isOwner ? `
                                    <div class="mt-2 text-end">
                                        ...
                                        <button class="btn btn-sm ${file.isPrivate ? 'btn-warning' : 'btn-outline-warning'} toggle-private-btn ms-1" data-file-id="${file.id}">
                                            <i class="bi ${file.isPrivate ? 'bi-lock-fill' : 'bi-unlock'}"></i>
                                        </button>
                                    </div>
                                ` : ''}
                            </div>
                ...
                // Agregar event listeners a los botones de privacidad
                document.querySelectorAll('.toggle-private-btn').forEach(btn => {
                    btn.addEventListener('click', function() {
                        toggleFilePrivacy(this.dataset.fileId);
                    });
                });
            })
            .catch(error => {
    ...
    // Alternar privacidad de un archivo
    function toggleFilePrivacy(fileId) {
        getFileById(fileId)
            .then(file => {
                return updateFile(fileId, { isPrivate: !file.isPrivate });
            })
            .then(() => {
                showToast('Éxito', 'Privacidad del archivo actualizada');
            ...
            .catch(error => {
                ...
                showToast('Error', 'No se pudo actualizar la privacidad del archivo', true);
            });
    }
    ...
    function loadUserFilesForSharing() {
        ...
        getUserFiles(currentUser.email)
            .then(files => {
                // Filtrar archivos que no sean privados
                files = files.filter(file => !file.isPrivate);
    ...
    function loadSharedFiles() {
        ...
        getAllFiles()
            .then(files => {
                // Filtrar archivos compartidos con el usuario actual y que no sean privados
                const sharedFiles = files.filter(file => 
                    file.sharedWith.includes(currentUser.email) && !file.isPrivate
                );
    ...
    const DB_VERSION = 3; // Incrementado para manejar cambios en el esquema (añadido isPrivate)
    ...
    function initDB() {
        return new Promise((resolve, reject) => {
            ...
            request.onupgradeneeded = function(event) {
                ...
                // Actualizar esquema si es necesario (para añadir isPrivate)
                if (oldVersion < 3) {
                    const transaction = event.target.transaction;
                    const fileStore = transaction.objectStore(FILE_STORE);
                    
                    // Agregar propiedad isPrivate a los archivos existentes
                    fileStore.getAll().onsuccess = function(e) {
